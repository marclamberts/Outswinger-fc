...
def display_landing_page():
    ...
    with col2:
        ...
        if os.path.exists(logo1_path) and os.path.exists(logo2_path):
            ...
            with lcol1: st.image(logo1, use_column_width=True)
            with lcol2: st.image(logo2, use_column_width=True)
        else:
            st.warning("Logos not found.")
    st.markdown("<br><br>", unsafe_allow_html=True)
    btn1, btn2, btn3, btn4 = st.columns(4, gap="medium")
    if btn1.button("Advanced Metrics"): st.session_state.app_mode = "Performance"
    if btn2.button("Matches"): st.session_state.app_mode = "Matches"
    if btn3.button("Player Shot Maps"): st.session_state.app_mode = "Profiles"
    if btn4.button("Corners"): st.session_state.app_mode = "SetPieces"

# --- Pages ---
def display_performance_page(metrics):
    st.subheader("Advanced Metrics / Player Scouting")
    if not metrics: st.warning("No metrics CSVs loaded."); return
...
def display_matches_page():
    ...
    with st.sidebar:
        ...
        if total_option=="Yes":
            ...
            team_map = all_matches[['TeamId']].drop_duplicates()
            ...
            # Generate subtitle
            other_teams = df_match['TeamId'].unique().tolist()
            opponent = [t for t in other_teams if t != team_choice]
            title_main = team_choice
...
def display_profiles_page(metrics):
    ...
    if 'WSL' not in metrics: st.warning("WSL metrics not loaded."); return
